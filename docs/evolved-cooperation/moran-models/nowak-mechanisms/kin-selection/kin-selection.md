...
The proof-of-mechanism results tell us whether cooperation reaches fixation from a rare start. The phase diagram tells us where in parameter space it succeeds. Neither shows the temporal dynamics — how fast the invasion proceeds, how consistent it is across seeds, and how the process differs when kin bias is present versus absent.

<figure style={{ margin: '0 0 1.5rem 0', textAlign: 'center' }}>
  <img
    src="/img/evolved-cooperation/kin-selection/kin_selection_invasion_trajectories.png"
    alt="Kin selection invasion trajectories: mean cooperation over 1000 steps from a rare start (5%), with kin bias vs. no kin bias ablation, 5 seeds per condition"
    style={{ maxWidth: '680px', width: '100%', border: '1px solid #D6E4F5' }}
  />
  <figcaption style={{ marginTop: '0.6rem' }}><strong>Display 5:</strong> Per-step mean cooperation for 5 seeds under two conditions starting from 5% cooperators. Thin lines = individual seeds; bold line = ensemble mean; shaded band = ±1 std. Blue: with kin bias (kin_weight_same=0.8, kin_weight_other=0.2, b/c=5). Red: no kin bias ablation (equal weights, well-mixed routing). Generated by <a href="https://github.com/doesburg11/EvolvedCooperation/blob/main/moran_models/nowak_mechanisms/kin_selection/utils/plot_invasion_trajectories.py">utils/plot_invasion_trajectories.py</a>.</figcaption>
</figure>
